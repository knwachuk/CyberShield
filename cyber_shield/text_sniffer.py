...
hatebert_analyzer = pipeline(
    "text-classification", model=hatebert_model, tokenizer=hatebert_tokenizer
)
    ...
    return {
        ...
        "vader": {
            "sentiment": (
                ...
                else "negative" if vader_result["compound"] < 0 else "neutral"
            ),
        ...
        "roberta": {
            "sentiment": (
                ...
                else "negative" if roberta_result["label"] == "LABEL_0" else "neutral"
            ),
        ...
        "textblob": {
            ...
            "sentiment": (
                "positive"
                if textblob_result.polarity > 0
                else "negative" if textblob_result.polarity < 0 else "neutral"
            ),
        },
    }
